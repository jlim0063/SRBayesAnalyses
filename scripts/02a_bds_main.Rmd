...
m.bds.ACC1<-glmer(
  IsCorrect~ 1 + Order + Condition + SessionTime + LogBaserate + LogLLA +  (1|PtID),
...
# Create log RTs
bds.data.WRSR.AM.exNB <- as.data.table(bds.data.WRSR.AM.exNB)
bds.data.WRSR.AM.exNB[, RT:=as.numeric(RT)]
bds.data.WRSR.AM.exNB[, logRT:=log(RT)]

bds.data.WRSR.PM.exNB <- as.data.table(bds.data.WRSR.PM.exNB)
...
m.bds.AM.rt <- lmer(logRT ~ 1 + Condition + entropy + LBRstrength + LLAstrength + (1 |PtID), data = bds.data.WRSR.AM.exNB)
m.bds.PM.rt <- lmer(logRT ~ 1 + Condition + entropy + LBRstrength + LLAstrength + (1|PtID), data = bds.data.WRSR.PM.exNB)

check_collinearity(m.bds.AM.rt)


```


Same as NSF analyses, test if no. blk balls at high levels of LBRstrength affects RT


```{r}

# Get distribution of high LBRstrength trials according to no. of black balls
table(bds.data.WRSR.AM.exNB[LBRstrength == 2.197224577]$NoBlkBall)/nrow(bds.data.WRSR.AM.exNB[LBRstrength == 2.197224577]) * 100
table(bds.data.WRSR.AM.exNB[LBRstrength == 2.197224577]$NoBlkBall)

# Get distirbution of high LogBaserate trials according to no. of black balls
table(bds.data.WRSR.AM.exNB[LogBaserate == 2.197224577]$NoBlkBall)
table(bds.data.WRSR.AM.exNB[LogBaserate == -2.197224577]$NoBlkBall)

# Black ball model
m.bds.bb <- lmer(logRT ~ 1 + factor(NoBlkBall) + (1|PtID), data = bds.data.WRSR.AM.exNB[LBRstrength == 2.197224577])
m.bds.bb2 <- lmer(logRT~ 1 + factor(NoBlkBall) + (1|PtID), data = bds.data.WRSR.AM.exNB[LogBaserate == 2.197224577])
...
bds.data.WRSR.AM <- as.data.table(bds.data.WRSR.AM)
bds.data.WRSR.AM[,LBRstrength:=abs(LogBaserate)]
bds.data.WRSR.AM[,LLAstrength:=abs(LogLLA)]
bds.data.WRSR.AM[,logRT := log(RT)]

test.rt <-lmer(logRT ~ 1 + LBRstrength + LLAstrength + Condition + (1 |PtID), data = bds.data.WRSR.AM)
